function [bestexperimento, posicoesBestExperimento,classic,  evobestFitness, trace] = aeiq_br_real3(param_real, numPulses, numClassic, ...
...
for g = 1:generations,
    ...
    for j=1:gap
          ...
          [wo_in{j,1},wo_back{j,1},wo{j,1},posicoes{j,1}] = geraPesos2(classicQreal(j,1),classicQreal(j,2),classicQreal(j,6),nInput,nOutput);
...
%     end
    ...
    if (g > 1)
        ...
        maskxQreal = rand(gap, 3) <= crossover;
     
        new_Qreal = zeros(size(maskxQreal,1),size(maskxQreal,2));
             
        idxCrossover = 1:numClassic;
        tmpBestQreal = bestQreal(idxCrossover(1:gap),3:5);
        
        tmpClassicQreal = classicQreal(:,3:5);
