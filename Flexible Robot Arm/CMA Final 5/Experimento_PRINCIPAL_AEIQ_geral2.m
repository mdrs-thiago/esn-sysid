...
numPulses = 5;     % população quântica
numClassic = 30;   % população clássica
genToWidth = 5;     % intervalo de gerações para atualização da população quântica
generations = 100;  % número de gerações
crossover = 0.6;    % taxa de crossover clássico real
qCrossover = 0.8;   % taxa de crossover quântico
showEv = 1;
gap = numClassic;          
seed = rng;
xanterior = [];
...
limInf = [50; 0.10; 0.0001; 0; 0.1]; % limites inferiores das variáveis
 
limSup = [400; 0.99; 1; 1; 1];   % limites superiores das variáveis

num_var = 5; % número de variáveis a serem otimizadas na etapa 1 de otimização de parâmetros da ESN
...
numClassic2 = 50;    % população clássica
genToWidth2 = 5;    % intervalo de gerações para atualização da população quântica
generations2 = 100;   % número de gerações
crossover2 = 0.6;  % taxa de crossover clássico real
...
figure('Name','Curva de Evolução - Etapa 1 AEIQ','NumberTitle','off');
...
for exp =1:numExp
  for i=1:numRedes     
     ...
     % Gerando uma ESN
     esn_totais{exp,i} = generate_esn5(nInputUnits, nInternalUnits, nOutputUnits, ScalWin, ScalWback, ...
...
for exp = 1:numExp
  ...
  esnP{exp} = generate_esn5(nInputUnits, nInternalUnits, nOutputUnits, ScalWin, ScalWback, ...
      'spectralRadius',raio,'inputScaling',1* ones(nInputUnits,1),'inputShift',0*ones(nInputUnits,1), ...
      'teacherScaling',1*ones(nOutputUnits,1),'teacherShift',0*ones(nOutputUnits,1),'feedbackScaling', 1, ...
      'type', 'leaky_esn','leakage',vazamento,...
      'reservoirActivationFunction','tanh',...
