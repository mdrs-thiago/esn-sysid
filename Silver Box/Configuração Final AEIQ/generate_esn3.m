 function esn = generate_esn3(nInputUnits, nInternalUnits, nOutputUnits,ScalWin, ScalWback, varargin)
...
esn.internalWeights_UnitSR = generate_internal_weights1(nInternalUnits, ...
                                                connectivity);
esn.nTotalUnits = nTotalUnits; 

% input weight matrix has weight vectors per input unit in colums
esn.inputWeights = (2.0 * rand(nInternalUnits, nInputUnits)- 1.0).*ScalWin;
...
esn.feedbackWeights = (2.0 * rand(nInternalUnits, nOutputUnits)- 1.0).*ScalWback;
