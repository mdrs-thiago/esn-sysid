...
for exp =1:numExp
  for i=1:numRedes     
     ...
     reservatorio = 150;  % número de neurônios no reservatório

     raio = 0.1;          % raio espectral
 
     ScalWin = 0.1;       % constante de multiplicação da matriz Win
 
     ScalWback = 0.1;     % constante de multiplicação da matriz Wback
 
     vazamento = 0.9;     % leaky rate dos neurônios leaky integrator

     conectividade = 0.7; % percentual de conectividade dos neurônios do reservatório
...
for exp =1:numExp
  ...
  % Previsão do conjunto de teste
%   predictedTestOutput = test_esn_modificada(testInputSequence, tr_esn_random{exp}, -200,'startingState',finalState);
  predictedTestOutput = test_esn(testInputSequence, tr_esn_random{exp}, 0);
  %predictedTestOutput = test_esn_modificada(testInputSequence, best_ESN, 0);
  yTestePrevisto = desnormaliza(predictedTestOutput,yMin,yMax);
  % Cálculo do RMSE_mV no conjunto de teste
...
predictedTestOutput = test_esn(testInputSequence, bestEsnRandom, 0);
yTestePrevisto = desnormaliza(predictedTestOutput,yMin,yMax);
