...
nForgetPoints = 1000;  % T0 - número de dados descartados durante o período de washout da ESN no treinamento

numRedes = 10;         % número de redes avaliadas por cada indivíduo na etapa 1 de otimização de parâmetros da ESN

metricaErro = 'RMSE_mV'; % metrica de erro a ser utilizada na função de aptidão
...
numClassic = 15;   % população clássica
genToWidth = 5;     % intervalo de gerações para atualização da população quântica
generations = 30;  % número de gerações
crossover = 0.3;    % taxa de crossover clássico real
qCrossover = 0.6;   % taxa de crossover quântico
showEv = 1;
gap = numClassic;          
seed = rng;
xanterior = [];
...
limInf = [200; 0.10; 0.0001; 0; 0.1;0.1]; % limites inferiores das variáveis
 
limSup = [450; 0.99; 1; 1; 1; 1];   % limites superiores das variáveis
...
numClassic2 = 30;    % população clássica
genToWidth2 = 5;    % intervalo de gerações para atualização da população quântica
generations2 = 30;   % número de gerações
crossover2 = 0.9;  % taxa de crossover clássico real
qCrossover2 = 0.4; % taxa de crossover quântico
gap2 = numClassic2;    
