function esn = generate_esn4(nInputUnits, nInternalUnits, nOutputUnits,ScalWin, ScalWback, Wo_in,Wo_back,Wo,varargin)
...
while success == 0
    ...
    try,
    opts.disp = 0;
    maxVal = max(abs(eigs(Wo,1, 'lm', opts)));
    esn.internalWeights_UnitSR = Wo/maxVal;
    success = 1 ;
    catch,
        success = 0 ; 
    end
...
esn.inputWeights = Wo_in.*ScalWin;
...
esn.feedbackWeights = Wo_back.*ScalWback;
